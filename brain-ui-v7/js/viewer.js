...
export class BrainViewer {
    ...
    generatePuzzleColors(count) {
        ...
        // Combine front and back brain colors based on Z-depth
        const colors = [];
        const halfCount = Math.ceil(count / 2);
        
        // Front half (rows 0-3) gets warm colors, back half (rows 4-7) gets cool colors
        for (let i = 0; i < count; i++) {
            const row = Math.floor(i / this.puzzleCols);
            if (row < this.puzzleRows / 2) {
                // Front rows - warm colors
                const idx = i % this.frontBrainColors.length;
                colors.push(this.frontBrainColors[idx]);
            } else {
                // Back rows - cool colors
        ...
        this.jigsawPieces.forEach((piece, index) => {
            ...
            if (piece === clickedPiece) {
                piece.userData.animation = {
                    ...
                    targetPos: new THREE.Vector3(0, 0, 3), // Move toward camera
                    targetRot: new THREE.Euler(0, 0, 0),
                    startScale: piece.scale.clone(),
                    targetScale: piece.scale.clone().multiplyScalar(8) // 8x larger = half screen
                };
