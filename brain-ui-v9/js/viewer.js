...
export class BrainViewer {
  ...
  - ${this.jigsawPieces.length} Jigsaw puzzle pieces with proper shapes (1.28x scale)`);
    ...
    animate() {
        ...
        // Update Matrix explosion animation
        if (this.matrixExploding) {
            ...
            this.matrixOverlay.forEach((mesh) => {
                if (mesh.userData.matrixAnim) {
                    ...
                    if (animElapsed > 0) {
                        const progress = Math.min(animElapsed / 2500, 1); // 2.5s duration
                        const easeOut = 1 - Math.pow(1 - progress, 3);
                        
                        // Spiral outward
                        const spiral = easeOut * anim.speed;
                        const spiralAngle = anim.angle + (easeOut * Math.PI * 3); // 3 rotations
                        
                        mesh.position.x = anim.startPos.x + Math.cos(spiralAngle) * spiral;
                        mesh.position.y = anim.startPos.y + Math.sin(spiralAngle) * spiral;
                        mesh.position.z = anim.startPos.z + (Math.random() - 0.5) * spiral * 0.5;
                        
                        // Spiral rotation
                        mesh.rotation.x = anim.startRot.x + anim.rotSpeed * easeOut;
                        mesh.rotation.y = anim.startRot.y + anim.rotSpeed * easeOut * 0.7;
                        mesh.rotation.z = anim.startRot.z + spiralAngle;
