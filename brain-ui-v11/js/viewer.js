  ...
  - ${this.jigsawPieces.length} Jigsaw puzzle pieces with proper shapes (1.28x scale)`);
    ...
    animate() {
        ...
        if (this.greenMelting) {
            ...
            const progress = Math.min(elapsed / 3000, 1); // 3s melt duration
            
            this.greenOverlay.forEach((mesh) => {
                if (mesh.userData.greenMelt) {
                    const anim = mesh.userData.greenMelt;
                    
                    // Get the Y position in local space
                    const localY = mesh.position.y;
                    
                    // Normalize Y to 0-1 range (brain roughly -1 to +1)
                    const normalizedY = (localY + 1.5) / 3.0; // 0 at bottom, 1 at top
                    const meltThreshold = 1 - progress; // Top melts first
                    
                    if (normalizedY > meltThreshold - 0.1) { // Small buffer
                        // This part is melting
                        const meltProgress = (normalizedY - meltThreshold + 0.1) / 0.1;
                        const meltAmount = Math.min(meltProgress, 1) * 3;
                        
                        // Drip downward
                        mesh.position.y = anim.startPos.y - meltAmount;
                        
                        // Fade out based on melt distance
                        const fadeProgress = Math.min(meltAmount / 1.5, 1);
                        
                        // Handle shader materials
                        if (mesh.material.uniforms) {
                            if (!mesh.material.userData.originalColors) {
                                mesh.material.userData.originalColors = {
                                    baseColor: mesh.material.uniforms.baseColor.value.clone(),
                                    glowColor: mesh.material.uniforms.glowColor.value.clone()
                                };
                            }
                            // Fade by reducing color intensity
                            const opacity = 1 - fadeProgress;
                            mesh.material.uniforms.baseColor.value.copy(
                                mesh.material.userData.originalColors.baseColor
                            ).multiplyScalar(opacity);
                            mesh.material.uniforms.glowColor.value.copy(
                                mesh.material.userData.originalColors.glowColor
                            ).multiplyScalar(opacity);
                        }
                        
                        // Hide when fully melted
                        if (fadeProgress >= 1) {
                            mesh.visible = false;
                        }
                    }
