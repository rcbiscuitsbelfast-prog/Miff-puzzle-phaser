...
export class BrainViewer {
  ...
  - ${this.jigsawPieces.length} Jigsaw puzzle pieces with proper shapes (1.28x scale)`);
    ...
    explodeAllPieces(clickedPiece = null) {
        ...
        this.jigsawPieces.forEach((piece, index) => {
            ...
            if (piece === clickedPiece) {
                ...
                piece.userData.animation = {
                    ...
                    duration: 4000, // 4 seconds to float and enlarge
                    isClickedPiece: true,
            ...
            // Store animation data with slower, more dramatic timing
            piece.userData.animation = {
                startTime: Date.now() + index * 50, // 50ms stagger (slower, more dramatic)
                duration: 2500, // Much longer duration for slower explosion
                startPos: piece.position.clone(),
                startRot: piece.rotation.clone(),
        ...
        // Start Matrix explosion after puzzle pieces are done (3.5s + 1s delay)
        setTimeout(() => {
    ...
    animate() {
        ...
        // Update Matrix drip animation
        if (this.matrixExploding) {
            const now = Date.now();
            const elapsed = now - this.matrixExplodeStart;
            
            this.matrixOverlay.forEach((mesh) => {
                if (mesh.userData.matrixAnim) {
                    const anim = mesh.userData.matrixAnim;
                    const animElapsed = elapsed - anim.delay;
                    
                    if (animElapsed > 0) {
                        const progress = Math.min(animElapsed / 3000, 1); // 3s drip duration
                        const easeIn = progress * progress; // Accelerating drip (gravity)
                        
                        // Drip downward with slight drift
                        mesh.position.y = anim.startPos.y - (easeIn * anim.dripSpeed * 8);
                        mesh.position.x = anim.startPos.x + (easeIn * anim.driftX);
                        mesh.position.z = anim.startPos.z;
                        
                        // Slight rotation as it drips
                        mesh.rotation.x = mesh.rotation.x + 0.01;
                        
                        // Fade out as it drips
                        if (progress > anim.fadeStart) {
                            const fadeProgress = (progress - anim.fadeStart) / (1 - anim.fadeStart);
        ...
        // Update green glow melt animation
        if (this.greenMelting) {
            const now = Date.now();
            const elapsed = now - this.greenMeltStart;
            const progress = Math.min(elapsed / 2500, 1); // 2.5s melt duration
            
            this.greenOverlay.forEach((mesh) => {
                if (mesh.userData.greenMelt) {
                    const anim = mesh.userData.greenMelt;
                    
                    // Get the Y position relative to the brain's bounding box
                    const worldPos = new THREE.Vector3();
                    mesh.getWorldPosition(worldPos);
                    
                    // Melt from top to bottom (Y position based)
                    // Normalize Y to 0-1 range (assuming brain is centered around 0)
                    const normalizedY = (worldPos.y + 2) / 4; // Adjust range as needed
                    const meltThreshold = 1 - progress; // Top melts first
                    
                    if (normalizedY > meltThreshold) {
                        // This part has melted
                        // Drip downward
                        const meltAmount = (normalizedY - meltThreshold) * 5;
                        mesh.position.y = anim.startPos.y - meltAmount;
                        
                        // Fade out
                        const fadeProgress = Math.min(meltAmount / 2, 1);
                        if (mesh.material && mesh.material.uniforms && mesh.material.uniforms.glowColor) {
                            // Shader material - adjust uniforms
                            const alpha = 1 - fadeProgress;
                            mesh.material.uniforms.glowColor.value.multiplyScalar(alpha);
                        } else if (mesh.material.opacity !== undefined) {
                            mesh.material.opacity = 1 - fadeProgress;
                        }
                        
                        // Hide completely when melted far enough
                        if (fadeProgress >= 1) {
                            mesh.visible = false;
                        }
                    }
