...
export class BrainViewer {
    ...
    generatePuzzleColors(count) {
        ...
        // Left half gets warm colors, right half gets cool colors
        for (let i = 0; i < count; i++) {
            const col = i % this.puzzleCols;
            if (col < this.puzzleCols / 2) {
                // Left side - warm colors
                const idx = i % this.leftBrainColors.length;
                colors.push(this.leftBrainColors[idx]);
            } else {
                // Right side - cool colors
                const idx = i % this.rightBrainColors.length;
                colors.push(this.rightBrainColors[idx]);
            }
        }
        ...
        brainModel.traverse((child) => {
            if (child.isMesh && child.geometry) {
                ...
                for (let i = 0; i < totalPieces; i++) {
                    ...
                    const pieceMaterial = new THREE.ShaderMaterial({
                        ...
                        fragmentShader: `
                            ...
                            void main() {
                                ...
                                // Get UV within cell for edge effects
                                vec2 cellUV = fract(gridCoord);
                                
                                // Add puzzle piece edges (darker borders)
                                ...
                                // Add subtle tab/blank visual hints
                                float midX = abs(cellUV.x - 0.5);
                                float midY = abs(cellUV.y - 0.5);
                                float tabHint = smoothstep(0.15, 0.25, midX) * smoothstep(0.45, 0.40, midY);
                                tabHint += smoothstep(0.15, 0.25, midY) * smoothstep(0.45, 0.40, midX);
                                
                                // Piece color with variations
                                vec3 color = pieceColor;
                                color *= (0.85 + cellUV.x * 0.15 + cellUV.y * 0.15); // Subtle gradient
                                color = mix(color, vec3(0.15), edge); // Darken edges
                                color = mix(color, color * 0.9, tabHint * 0.3); // Tab shadows
                                
                                gl_FragColor = vec4(color, 1.0); // FULL OPACITY - no gaps!
                            }
                        `,
                    ...
                    pieceMesh.scale.copy(child.scale).multiplyScalar(1.40); // Slightly larger for full coverage
